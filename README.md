...
| `/context` | Inspect the actual model/route, saved branch and retained conversation count; show only provider-reported input/output tokens from OpenAI Responses, Anthropic Messages, Google Gemini or Ollama on the selected ancestry (or cumulative ephemeral usage), with other routes, context limit and cost untracked |
