...
The default provider uses `OPENAI_API_KEY` when making a request. The interactive terminal opens before credentials are configured; use `/login` and `/model` from there. `pave --providers` lists available routes. `--models` queries pinned account/provider listings for OpenAI, Gemini, Anthropic (API key), DeepSeek, Groq, Mistral, Together, Cerebras, Venice, DeepInfra, Fireworks, Baseten, Hugging Face, NanoGPT, AIML API, ai&, Sakana, Abliteration, GMI Cloud, Amazon Bedrock foundations, Codex, OpenRouter and personal Copilot, plus local Ollama, LM Studio, llama.cpp and vLLM hosts. Azure deployments and Google Vertex model IDs require explicit configuration; their model discovery is not available. No model ID is built into the provider catalog. The cancellable `/model` picker displays discovered IDs; if discovery is unavailable, type a model ID explicitly. A listing does not guarantee account entitlement or tool support. Keep API keys out of checked-in config and session files.
