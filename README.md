...
Setup uses the same searchable keyboard picker as `/model`, with concise step guidance on wide terminals and a compact fallback on narrow terminals. Resize preserves the active choice; the guidance is presentation only, not saved conversation content.
