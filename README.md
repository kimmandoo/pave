...
Setup uses the same searchable keyboard picker and asynchronous account/local model discovery as `/model`. After authentication, the chooser automatically lists available verified live IDs first; use Up/Down and Enter to select one without typing an ID. Offline suggestions remain available if discovery is unsupported or fails; the status says why, and a manually typed route-compatible `PROVIDER/MODEL_ID` remains an explicit fallback with unverified entitlement. Available discovery covers API-key OpenAI/Google, local Ollama, signed-in personal Copilot and account-scoped Codex subscription OAuth; other OAuth providers are not yet listed live. Codex listing reuses inference's locked token refresh and sends its bearer/account ID only to pinned ChatGPT backend routes. Resize preserves the active choice; wide terminals show step guidance and narrow terminals stay compact.
