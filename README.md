...
These 45 providers have isolated wire and tool-result fixtures, not live vendor account calls; the sibling rules declare 83 distinct provider identities, so 38 remain unsupported. Six additional native routes—Synthetic, Z.AI standard Chat, ZenMux, Wafer Serverless, Qianfan V2 and Xiaomi pay-as-you-go—passed pinned fake-HTTPS native CLI scenarios with real workspace `read_file` results. The Z.AI Coding Plan Anthropic route, Xiaomi regional Token Plans and Z.AI model listing are not implemented. Qianfan discovery lists only explicitly `type=chat` IDs; StepFun, Synthetic, ZenMux, Wafer and Xiaomi IDs are unclassified in the TUI because their listings do not prove Chat/tool capability. Amazon Bedrock SigV4 Converse and Bedrock Mantle bearer Responses are separate working routes; AWS SSO/roles, inference-profile discovery and binary ConverseStream are missing. Vertex requires explicit project/location/model and supports explicit Google token, gcloud ADC or metadata token; managed OAuth clients, model discovery and Claude routes are missing. Neither a provider listing nor these simulated turns proves live account entitlement or every listed model's tool support.
