...
The default provider uses `OPENAI_API_KEY` when making a request. The interactive terminal opens before credentials are configured; use `/login` and `/model` from there. `pave --providers` lists available routes. `--models` queries pinned account/provider listings for OpenAI, Gemini, Anthropic (API key), DeepSeek, Groq, Mistral, Together, Cerebras, Venice, DeepInfra, Fireworks, Baseten, Hugging Face, NanoGPT, AIML API, ai&, Sakana, Abliteration, GMI Cloud, Moonshot AI, Ollama Cloud, xAI, NVIDIA hosted NIM, Amazon Bedrock foundations, Bedrock Mantle, Codex, OpenRouter and personal Copilot, plus local Ollama, LM Studio, llama.cpp and vLLM hosts. Azure deployments and Google Vertex model IDs require explicit configuration; their model discovery is not available. No model ID is built into the provider catalog. The cancellable `/model` picker displays discovered IDs; if discovery is unavailable, type a model ID explicitly. A listing does not guarantee account entitlement, Responses compatibility or tool support. Keep API keys out of checked-in config and session files.
...
Setup uses the same searchable keyboard picker and asynchronous account/local model discovery as `/model`. After authentication, the chooser lists returned live IDs first; use Up/Down and Enter to select one without typing an ID. If discovery is unsupported or fails, the status says why and you may type a route-compatible `PROVIDER/MODEL_ID` explicitly; its entitlement remains unverified. Discovery supports API-key OpenAI/Google/Anthropic/DeepSeek/Groq/Mistral/Together/Cerebras/Venice/DeepInfra/Fireworks/Baseten/Hugging Face/NanoGPT/AIML API/ai&/Sakana/Abliteration/GMI Cloud/Moonshot/Ollama Cloud/xAI/NVIDIA/Bedrock Mantle, AWS-signed Bedrock foundation models, local Ollama/LM Studio/llama.cpp/vLLM, signed-in personal Copilot, account-scoped Codex OAuth and OpenRouter's authenticated user-filtered catalog. AIML API and ai& filter advertised tool-capable models; Sakana, GMI Cloud, Moonshot, Ollama Cloud, xAI, NVIDIA, Bedrock Converse and Mantle listing rows do not certify every model's tool support, correct wire route or account invocation rights. Anthropic and Fireworks cursor pages are bounded and malformed pages fail rather than showing partial results. Codex listing reuses inference's locked token refresh; OAuth credentials go only to pinned backend routes. Azure and Vertex require explicit deployment/model IDs. Resize preserves the active choice; wide terminals show guidance and narrow terminals stay compact.
