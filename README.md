...
The default provider uses `OPENAI_API_KEY` when making a request. The interactive terminal opens before credentials are configured; use `/login` and `/model` from there. `pave --providers` lists available routes. `--models` queries pinned account/provider listings for OpenAI, Gemini, Anthropic (API key), DeepSeek, Groq, Mistral, Together, Cerebras, Venice, DeepInfra, Fireworks, Baseten, Hugging Face, NanoGPT, AIML API, ai&, Sakana, Abliteration, GMI Cloud, Moonshot AI, Ollama Cloud, xAI, NVIDIA hosted NIM, Novita AI, SiliconFlow global/China, W&B-hosted CoreWeave Serverless, StepFun international, Synthetic, ZenMux, Wafer Serverless, Qianfan V2, Xiaomi MiMo, Amazon Bedrock foundations, Bedrock Mantle, Codex, OpenRouter and personal Copilot, plus local Ollama, LM Studio, llama.cpp and vLLM hosts. Azure deployments, Google Vertex and standard Z.AI Chat model IDs require explicit configuration; their model discovery is not documented here. No model ID is built into the provider catalog. The cancellable `/model` picker shows authenticated Chat-classified IDs; StepFun, Synthetic, ZenMux, Wafer and Xiaomi listings lack a verified Chat/tool classification and require typing a known Chat model ID. A listing does not guarantee account entitlement, Responses compatibility or tool support. Keep API keys out of checked-in config and session files.
...
StepFun international uses `STEPFUN_API_KEY` only on `api.stepfun.ai`, not the distinct `.com` platform. Its `/v1/models` lists non-Chat audio IDs without modality or tool flags: `--models` labels them unclassified, and `/model` does not falsely offer them as verified Chat choices; select an explicitly known Chat ID. The native transport accepts StepFun's documented `finish_reason: "stop"` with complete tool calls without weakening other Chat providers' strict finish validation. CoreWeave Serverless uses W&B Inference's fixed host, `COREWEAVE_API_KEY` or `WANDB_API_KEY`, and authenticated account `/v1/models`; a CoreWeave control-plane credential is not interchangeable with a W&B Inference key. Both routes buffer validated completions and passed isolated native CLI two-turn real workspace tool-result scenarios; live vendor account support was not verified.

Inside a running Pave terminal, `/login` connects an OAuth/device-code account **without changing the active model or default**; after sign-in, choose whether to pick a model for this conversation or keep the current one. `/setup` is the guided provider → access → API/model flow and saves a **user default**; `/model` switches only the current conversation, and `/settings` edits project defaults for the next launch. API-key providers use environment variables rather than `/login`. Browser URLs and verification codes appear on the regular terminal while the full-screen UI is suspended, then the previous transcript/editor returns. `/model` loads the selected provider's models asynchronously into a searchable keyboard picker. Pick a listed ID with arrows and Enter, or type `PROVIDER/MODEL_ID` if discovery is unavailable. For providers requiring an explicit wire route, type `/model commandcode@messages/MODEL_ID` (or choose an API in setup/settings); the header shows the chosen route and saved sessions restore it. Escape cancels the outstanding listing without submitting the draft. A bare ID uses the current provider and API; everything after the first slash belongs to the model ID. Switching models keeps visible conversation; signed provider output is retained only for a matching provider, model and API route.
...
Setup uses the same searchable keyboard picker and asynchronous account/local model discovery as `/model`. After authentication, the chooser lists returned live IDs first; use Up/Down and Enter to select one without typing an ID. If discovery is unsupported or fails, the status says why and you may type a route-compatible `PROVIDER/MODEL_ID` explicitly; its entitlement remains unverified. Discovery supports API-key OpenAI/Google/Anthropic/DeepSeek/Groq/Mistral/Together/Cerebras/Venice/DeepInfra/Fireworks/Baseten/Hugging Face/NanoGPT/AIML API/ai&/Sakana/Abliteration/GMI Cloud/Moonshot/Ollama Cloud/xAI/NVIDIA/Novita/SiliconFlow global/China/W&B-hosted CoreWeave/Bedrock Mantle, AWS-signed Bedrock foundation models, local Ollama/LM Studio/llama.cpp/vLLM, signed-in personal Copilot, account-scoped Codex OAuth and OpenRouter's authenticated user-filtered catalog. StepFun's `/models` mixes Chat/non-Chat IDs and its TUI picker warns rather than offering them as verified Chat suggestions; type an explicitly known Chat ID. AIML API and ai& filter advertised tool-capable models; most other listing rows do not certify every model's tool support, correct wire route or account invocation rights. Anthropic and Fireworks cursor pages are bounded and malformed pages fail rather than showing partial results. Codex listing reuses inference's locked token refresh; OAuth credentials go only to pinned backend routes. Azure and Vertex require explicit deployment/model IDs. Resize preserves the active choice; wide terminals show guidance and narrow terminals stay compact.
...
| `/login [PROVIDER]` · `/model [PROVIDER[@API]/MODEL_ID]` · `/setup` | Connect an account without automatically switching models · choose the active conversation model/API · save a guided user default |
...
| Kilo | native Chat; keyless public model IDs unclassified | optional `KILO_API_KEY` | `--provider kilo --model KNOWN_CHAT_ID` |
...
| GitLab Duo Direct Access | account-bound token exchange then Anthropic, Responses or Chat proxy | `GITLAB_TOKEN` PAT; OAuth login not wired | `--provider gitlab-duo --api messages|responses|chat --model KNOWN_UPSTREAM_MODEL_ID` |
| Devin CLI | pinned protobuf/Connect Chat and account-scoped models | `DEVIN_API_KEY` session token; OAuth login not wired | `--provider devin --models`, then `--model ACCOUNT_MODEL_ID` |
