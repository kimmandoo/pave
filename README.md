...
The default provider uses `OPENAI_API_KEY` when making a request. The interactive terminal opens before credentials are configured; use `/login` and `/model` from there. `pave --providers` lists available routes. `--models` queries pinned account/provider listings for OpenAI, Gemini, Anthropic (API key), DeepSeek, Groq, Mistral, Together, Cerebras, Venice, DeepInfra, Fireworks, Baseten, Hugging Face, NanoGPT, AIML API, ai&, Sakana, Abliteration, GMI Cloud, Moonshot AI, Ollama Cloud, xAI, NVIDIA hosted NIM, Novita AI, SiliconFlow global/China, W&B-hosted CoreWeave Serverless, StepFun international, Amazon Bedrock foundations, Bedrock Mantle, Codex, OpenRouter and personal Copilot, plus local Ollama, LM Studio, llama.cpp and vLLM hosts. Azure deployments and Google Vertex model IDs require explicit configuration; their model discovery is not available. No model ID is built into the provider catalog. The cancellable `/model` picker displays discovered IDs when their Chat route is established; StepFun IDs remain unclassified. A listing does not guarantee account entitlement, Responses compatibility or tool support. Keep API keys out of checked-in config and session files.
...
These twenty-nine entries have **isolated transport/CLI fixtures**, not live vendor calls; the sibling rules declare 83 distinct providers, so 54 remain unimplemented. Newly added Abliteration Responses and GMI Cloud Chat passed native CLI two-request tool/result fixtures using real workspace `read_file` output and pinned fake HTTPS. Bedrock Converse completed a signed regional native CLI two-request fixture and signed `ListFoundationModels` lookup; Vertex completed a native two-request Gemini SSE fixture with preserved signed tool state and real workspace tool output. AWS SigV4 authentication supports static environment/shared profile credentials only; SSO/roles, inference-profile discovery, binary ConverseStream and Bedrock Mantle are missing. Vertex requires explicit project/location/model and supports an explicit Google token, gcloud ADC or metadata token; managed OAuth clients, model discovery and Claude routes are missing. A Bedrock foundation listing is not evidence of InvokeModel access, and GMI listing does not report per-model tool capability. Existing Codex, OpenRouter, Gemini, Copilot, Azure, Sakana, local servers and account-gateway regression fixtures remain covered by the full suite. Live provider entitlement and actual vendor responses are **unverified**; no compatible route implies all model features work.
