...
Setup uses the same searchable keyboard picker and asynchronous account/local model discovery as `/model`, with concise step guidance on wide terminals and a compact fallback on narrow terminals. Verified live IDs are distinct from offline suggestions; if listing is unavailable you can still type a route-compatible `PROVIDER/MODEL_ID`, but its entitlement is unverified. Resize preserves the active choice; guidance is presentation only, not saved conversation content.
