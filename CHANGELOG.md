...
## 2026-09-25
- feat(tui): add `/usage` to the typed command catalog and searchable help; private journals show real provider/model-grouped token totals only on the selected ancestry, while unsaved conversations show an Ollama-only combined total. A 70×18 native PTY verified empty saved/ephemeral states, two streamed model turns showing 36 input/14 output tokens, and headless `/help` exposed the command. Journal tests cover aggregation, fork isolation and branch rollback.
