(* International StepFun Open Platform; keys from platform.stepfun.ai must stay
   on api.stepfun.ai, not the separate api.stepfun.com platform.
   https://platform.stepfun.ai/docs/en/api-reference/models/list
   https://platform.stepfun.ai/docs/en/api-reference/models/object
   https://platform.stepfun.ai/docs/en/api-reference/chat/chat-completion-create
   https://platform.stepfun.ai/docs/en/api-reference/tool-call
   Model objects expose no modality or tool-capability metadata. Discovery IDs
   are therefore unclassified; an ID is not a claim of Chat/tool support. *)
let models_url = "https://api.stepfun.ai/v1/models"
let chat_url = "https://api.stepfun.ai/v1/chat/completions"
let max_response_bytes = 1_048_576
let max_models = 4096

type error =
  | Invalid_credential
  | Transport_error
  | Http_error of int
  | Invalid_response of string

type http = url:string -> headers:(string * string) list ->
  (int * string, error) result

let valid_key key = key <> "" && String.length key <= 8192 &&
  String.for_all (fun c -> Char.code c > 32 && Char.code c < 127) key

let env_api_key () = match Sys.getenv_opt "STEPFUN_API_KEY" with
  | Some key when valid_key key -> Some key
  | _ -> None

let chat_headers ~endpoint ~api_key =
  if endpoint <> chat_url then
    invalid_arg "StepFun credential requires the official international Chat endpoint";
  if not (valid_key api_key) then invalid_arg "invalid StepFun API key";
  ["Authorization: Bearer " ^ api_key]

let valid_id id = id <> "" && String.length id <= 256 &&
  String.for_all (fun c -> Char.code c > 32 && Char.code c < 127) id

let parse_models body =
  let json = try Some (Yojson.Basic.from_string body)
    with Yojson.Json_error _ -> None in
  match json with
  | Some (`Assoc fields) ->
      (match List.assoc_opt "data" fields with
      | Some (`List rows) when List.length rows <= max_models ->
          let seen = Hashtbl.create (List.length rows) in
          let ids = ref [] in
          let valid = List.for_all (function
            | `Assoc fields ->
                (match List.assoc_opt "id" fields,
                  List.assoc_opt "object" fields with
                | Some (`String id), Some (`String "model") when valid_id id ->
                    if not (Hashtbl.mem seen id) then (
                      Hashtbl.add seen id ();
                      ids := id :: !ids);
                    true
                | _ -> false)
            | _ -> false) rows in
          if valid then Ok (List.rev !ids)
          else Error (Invalid_response "invalid StepFun model object")
      | Some (`List _) -> Error (Invalid_response "too many StepFun models")
      | _ -> Error (Invalid_response "missing data array"))
  | _ -> Error (Invalid_response "malformed StepFun model listing")

(* Production's HTTPS executor must cap downloads and never follow redirects;
   this helper also rejects an oversized injected HTTP response. *)
let discover ~http ~api_key () =
  if not (valid_key api_key) then Error Invalid_credential
  else
    let headers = ["Authorization", "Bearer " ^ api_key;
      "Accept", "application/json"] in
    match http ~url:models_url ~headers with
    | Error failure -> Error failure
    | Ok (status, _) when status < 200 || status >= 300 ->
        Error (Http_error status)
    | Ok (_, body) when String.length body > max_response_bytes ->
        Error (Invalid_response "listing exceeds size limit")
    | Ok (_, body) -> parse_models body

(* StepFun's default reasoning_format is general (reasoning); deepseek-style
   yields reasoning_content. Neither format nor reasoning_effort is forced:
   supported modes and tool capability depend on the selected model. *)
let reasoning_fields json = match json with
  | `Assoc fields ->
      List.filter_map (fun key -> match List.assoc_opt key fields with
        | None | Some `Null -> None
        | Some (`String _ as value) -> Some (key, value)
        | Some _ -> raise (Protocol.Invalid_response ("invalid StepFun " ^ key)))
        ["reasoning"; "reasoning_content"]
  | _ -> raise (Protocol.Invalid_response "invalid StepFun assistant state")

let request ~model messages tools =
  let message (msg : Protocol.message) =
    let json = Protocol.message_to_json msg in
    match json with
    | `Assoc fields when msg.role = "assistant" ->
        let fields = if msg.content = None then fields @ ["content", `Null]
          else fields in
        let fields = match msg.provider_state with
          | None -> fields
          | Some state -> fields @ reasoning_fields state in
        `Assoc fields
    | _ -> json in
  let fields = ["model", `String model;
    "messages", `List (List.map message messages);
    "stream", `Bool false] in
  `Assoc (if tools = [] then fields else fields @ ["tools", `List tools])

let parse_completion json =
  let choice = match Protocol.member "choices" json with
    | `List (choice :: _) -> choice
    | _ -> raise (Protocol.Invalid_response "missing StepFun choices") in
  let message = Protocol.member "message" choice in
  let calls = Protocol.member "tool_calls" message in
  let has_calls = match calls with
    | `List (_ :: _ as calls) ->
        (* Every call must be a complete function invocation with JSON-object
           arguments before any of its results can be executed. *)
        List.iter (fun call ->
          let function_json = Protocol.member "function" call in
          let arguments = Protocol.member "arguments" function_json in
          match Protocol.member "type" call,
            Protocol.member "id" call,
            Protocol.member "name" function_json, arguments with
          | `String "function", `String id, `String name, `String args
            when id <> "" && name <> "" ->
              (match (try Some (Yojson.Basic.from_string args)
                with Yojson.Json_error _ -> None) with
              | Some (`Assoc _) -> ()
              | _ -> raise (Protocol.Invalid_response
                  "invalid StepFun function arguments"))
          | _ -> raise (Protocol.Invalid_response
              "incomplete StepFun function call")) calls;
        true
    | _ -> false in
  (* The official Tool Call example uses finish_reason="stop" even when the
     assistant returned tool_calls. Accept that exact vendor shape only;
     retain the generic parser's strict finish checks everywhere else. *)
  let checked = if has_calls &&
      Protocol.member "finish_reason" choice = `String "stop"
    then match json, Protocol.member "choices" json, choice with
      | `Assoc fields, `List (_ :: remaining), `Assoc selected ->
          let normalized = `Assoc (("finish_reason", `String "tool_calls") ::
            List.remove_assoc "finish_reason" selected) in
          `Assoc (("choices", `List (normalized :: remaining)) ::
            List.remove_assoc "choices" fields)
      | _ -> json
    else json in
  let result = Protocol.parse_completion checked in
  match reasoning_fields message with
  | [] -> result
  | fields -> { result with provider_state = Some (`Assoc fields) }
