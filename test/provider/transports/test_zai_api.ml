module Zai = Pave.Zai_api
module Protocol = Pave.Protocol
let field = Protocol.member

let key = "private-zai-fixture"
let model = "customer-selected-model"
let call_id = "call_zai_47"
...
let reasoning = "Use the function to calculate the product."
let user = `Assoc ["role", `String "user";
  "content", `String "What is six times seven?"]
let fail reason = failwith ("Z.AI fixture: " ^ reason)

(* Self as curl verifies the production HTTPS POST path, authenticates only
   the pinned host, and reads both complete request bodies across turns. *)
let fake_curl () =
  let config = ref [] in
  ...
  let state = Sys.getenv "PAVE_ZAI_FIXTURE_STATE" in
  ...
  let response = match step with
    | 0 ->
        ...
        {|{"id":"zai-1","object":"chat.completion","choices":[{"index":0,"finish_reason":"tool_calls","message":{"role":"assistant","content":null,"reasoning_content":"Use the function to calculate the product.","tool_calls":[{"id":"call_zai_47","type":"function","function":{"name":"multiply_seven","arguments":"{\"number\":6}"}}]}}]}|}
    | 1 ->
        ...
        | _ -> fail "missing assistant call or matching tool result");
        {|{"id":"zai-2","object":"chat.completion","choices":[{"index":0,"finish_reason":"stop","message":{"role":"assistant","content":"Six times seven is 42."}}]}|}
...
let () =
  if Array.length Sys.argv >= 3 && Sys.argv.(1) = "--disable" && Sys.argv.(2) = "--config" then
    ...
    assert (Zai.chat_headers ~endpoint:Zai.chat_url ~api_key:key =
      ["Authorization: Bearer " ^ key]);
    List.iter (fun endpoint ->
      match Zai.chat_headers ~endpoint ~api_key:key with
      | exception Invalid_argument _ -> ()
      | _ -> fail "credential escaped the pinned Chat endpoint") [
      "https://evil.example/api/paas/v4/chat/completions";
      "https://api.z.ai.evil.example/api/paas/v4/chat/completions";
      "http://api.z.ai/api/paas/v4/chat/completions";
      "https://api.z.ai/api/anthropic/v1/messages"];
    List.iter (fun bad ->
      match Zai.chat_headers ~endpoint:Zai.chat_url ~api_key:bad with
      | exception Invalid_argument _ -> ()
      | _ -> fail "invalid credential accepted") [""; "bad\r\nAuthorization: Bearer stolen"];
    let directory = Filename.temp_file "pave-zai-" "" in
    ...
    Fun.protect ~finally:(fun () ->
      ...
      Unix.putenv "PAVE_ZAI_FIXTURE_STATE" "";
      List.iter (fun path -> if Sys.file_exists path then Sys.remove path) [binary; state];
      Unix.rmdir directory) (fun () ->
      Unix.putenv "PATH" (directory ^ ":" ^ old_path);
      Unix.putenv "PAVE_ZAI_FIXTURE_STATE" state;
      let config : Pave.Provider.config = {
        endpoint = Zai.chat_url; api_key = key; model;
        api = Pave.Provider.Zai_chat } in
      (match Pave.Provider.complete
        { config with endpoint = "https://api.z.ai.evil.example/api/paas/v4/chat/completions" }
      ...
      let call = match first.tool_calls with
        ...
        | _ -> fail "Z.AI tool call not decoded" in
    ...
    print_endline "Z.AI pinned Chat reasoning/tool continuation: ok")
