module Coreweave = Pave.Coreweave_api
module Protocol = Pave.Protocol
let field = Protocol.member

let key = "private-coreweave-fixture"
let model = "newvendor/future-chat-model"
let call_id = "call_coreweave_42"
...
let fail reason = failwith ("CoreWeave fixture: " ^ reason)
...
let expect_error check = function
  ...
  | _ -> fail "unsafe CoreWeave listing accepted"
let invalid = function Coreweave.Invalid_response _ -> true | _ -> false

(* Inspect the production curl GET and both POST requests, including the
   authenticated model list and real tool-result continuation. *)
let fake_curl () =
  let config = ref [] in
  ...
  let state = Sys.getenv "PAVE_COREWEAVE_FIXTURE_STATE" in
  ...
  let response = match step with
    | 0 ->
        has "url" Coreweave.models_url;
        has "request" "GET";
        has "max-filesize" (string_of_int Coreweave.max_response_bytes);
        assert (List.mem "Accept: application/json" (values "header"));
        {|{"object":"list","data":[{"id":"newvendor/future-chat-model","object":"model","created":0}]}|}
    | 1 | 2 ->
        has "url" Coreweave.chat_url;
        has "request" "POST";
        ...
        (match body with
         | `Assoc fields ->
             ...
             assert (not (List.mem_assoc "chat_template_kwargs" fields));
             assert (not (List.mem_assoc "max_tokens" fields))
         | _ -> fail "Chat request is not an object");
        if step = 1 then (
          assert (field "messages" body = `List [user]);
          {|{"id":"cw-1","object":"chat.completion","choices":[{"index":0,"finish_reason":"tool_calls","message":{"role":"assistant","content":null,"reasoning":"I need to calculate this product.","tool_calls":[{"id":"call_coreweave_42","type":"function","function":{"name":"multiply_seven","arguments":"{\"number\":6}"}}]}}]}|})
        else (
          ...
          {|{"id":"cw-2","object":"chat.completion","choices":[{"index":0,"finish_reason":"stop","message":{"role":"assistant","content":"Six times seven is 42.","reasoning":null}}]}|})
...
let () =
  if Array.length Sys.argv >= 3 && Sys.argv.(1) = "--disable" && Sys.argv.(2) = "--config" then
    ...
    let previous_coreweave = Sys.getenv_opt "COREWEAVE_API_KEY" in
    let previous_wandb = Sys.getenv_opt "WANDB_API_KEY" in
    Fun.protect ~finally:(fun () ->
      Unix.putenv "COREWEAVE_API_KEY" (Option.value ~default:"" previous_coreweave);
      Unix.putenv "WANDB_API_KEY" (Option.value ~default:"" previous_wandb)) (fun () ->
      Unix.putenv "COREWEAVE_API_KEY" key;
      Unix.putenv "WANDB_API_KEY" "fallback-wandb-key";
      assert (Coreweave.env_api_key () = Some key);
      Unix.putenv "COREWEAVE_API_KEY" "";
      assert (Coreweave.env_api_key () = Some "fallback-wandb-key");
      Unix.putenv "COREWEAVE_API_KEY" "bad\nkey";
      assert (Coreweave.env_api_key () = Some "fallback-wandb-key"));
    let calls = ref 0 in
    ...
    expect_error ((=) Coreweave.Invalid_credential)
      (Coreweave.discover ~http ~api_key:"bad\r\nAuthorization: Bearer stolen" ());
    assert (!calls = 1);
    ...
    expect_error ((=) (Coreweave.Http_error 302)) (Coreweave.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (302, "redirect")) ~api_key:key ());
    assert (Coreweave.chat_headers ~endpoint:Coreweave.chat_url ~api_key:key =
      ["Authorization: Bearer " ^ key]);
    (match Coreweave.chat_headers ~endpoint:"https://evil.example/v1/chat/completions" ~api_key:key with
     | exception Invalid_argument _ -> ()
     | _ -> fail "credential escaped pinned host");
    (match Coreweave.chat_headers ~endpoint:Coreweave.chat_url ~api_key:"bad\nvalue" with
     | exception Invalid_argument _ -> ()
     | _ -> fail "header injection accepted");
    let directory = Filename.temp_file "pave-coreweave-" "" in
    ...
    Fun.protect ~finally:(fun () ->
      ...
      Unix.putenv "PAVE_COREWEAVE_FIXTURE_STATE" "";
      ...
      let config : Pave.Provider.config = {
        ...
        api = Pave.Provider.Coreweave_chat } in
      ...
      let call = match first.tool_calls with
        ...
        | _ -> fail "CoreWeave tool call not decoded" in
      assert (first.provider_state = Some (`Assoc [
        "reasoning", `String "I need to calculate this product."]));
    ...
    print_endline "CoreWeave authenticated discovery and tool continuation: ok")
