...
let expect_error check = function
  ...
  | _ -> fail "unsafe Novita listing accepted"
let invalid = function Novita.Invalid_response _ -> true | _ -> false

(* Replacing curl inspects the production request path, including HTTPS,
   credentials, bounded listing, and the actual two-turn Chat payload. *)
let fake_curl () =
  let config = ref [] in
  ...
  let state = Sys.getenv "PAVE_NOVITA_FIXTURE_STATE" in
  ...
  let response = match step with
    | 0 ->
        has "url" Novita.models_url;
        has "request" "GET";
        has "max-filesize" (string_of_int Novita.max_response_bytes);
        assert (List.mem "Accept: application/json" (values "header"));
        {|{"data":[{"id":"example/future-chat-model","object":"model","context_size":64000}]}|}
    | 1 | 2 ->
        has "url" Novita.chat_url;
        has "request" "POST";
        ...
        (match body with
         | `Assoc fields ->
             ...
             assert (not (List.mem_assoc "separate_reasoning" fields));
             assert (not (List.mem_assoc "max_tokens" fields))
         | _ -> fail "Chat request is not an object");
        if step = 1 then (
          assert (field "messages" body = `List [user]);
          {|{"id":"novita-1","object":"chat.completion","choices":[{"index":0,"finish_reason":"tool_calls","message":{"role":"assistant","content":null,"reasoning_content":"I need the tool to calculate the product.","reasoning_details":[{"type":"reasoning.text","format":"openai-responses-v1","id":"reasoning-text-1","index":0,"text":"I need the tool to calculate the product."}],"tool_calls":[{"id":"call_novita_47","type":"function","function":{"name":"multiply_seven","arguments":"{\"number\":6}"}}]}}]}|})
        else (
          ...
          {|{"id":"novita-2","object":"chat.completion","choices":[{"index":0,"finish_reason":"stop","message":{"role":"assistant","content":"Six times seven is 42."}}]}|})
...
let () =
  if Array.length Sys.argv >= 3 && Sys.argv.(1) = "--disable" && Sys.argv.(2) = "--config" then
    ...
    expect_error ((=) Novita.Invalid_credential)
      (Novita.discover ~http ~api_key:"bad\r\nAuthorization: Bearer stolen" ());
    assert (!calls = 1);
    List.iter (fun body -> expect_error invalid
      (Novita.discover ~http:(fun ~url:_ ~headers:_ -> Ok (200, body))
         ~api_key:key ())) [
      {|{"data":[{"id":"bad\nmodel"}]}|};
      {|{"data":[{"id":null}]}|};
      {|{"data":[{}]}|};
    ...
    expect_error invalid (Novita.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (200,
        String.make (Novita.max_response_bytes + 1) 'x')) ~api_key:key ());
    expect_error ((=) (Novita.Http_error 302)) (Novita.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (302, "redirect")) ~api_key:key ());
    assert (Novita.chat_headers ~endpoint:Novita.chat_url ~api_key:key =
      ["Authorization: Bearer " ^ key]);
    (match Novita.chat_headers ~endpoint:"https://evil.example/v1/chat/completions" ~api_key:key with
     | exception Invalid_argument _ -> ()
     | _ -> fail "credential escaped pinned host");
    (match Novita.chat_headers ~endpoint:Novita.chat_url ~api_key:"bad\nvalue" with
     | exception Invalid_argument _ -> ()
     | _ -> fail "header injection accepted");
    let directory = Filename.temp_file "pave-novita-" "" in
    ...
    Fun.protect ~finally:(fun () ->
      ...
      let call = match first.tool_calls with
        ...
        | _ -> fail "Novita tool call not decoded" in
    ...
    print_endline "Novita authenticated listing and reasoning/tool continuation: ok")
