module Zenmux = Pave.Zenmux_api
module Protocol = Pave.Protocol
let field = Protocol.member

let key = "private-zenmux-fixture"
let model = "example/future-chat-model"
let call_id = "toolu_bdrk_01ZenMuxProof"
...
let details = `List [`Assoc ["type", `String "reasoning.text";
  "text", `String reasoning;
  "signature", `String "signed-private-reasoning-proof";
...
let fail reason = failwith ("ZenMux fixture: " ^ reason)
...
let expect_error check = function
  ...
  | _ -> fail "unsafe ZenMux listing accepted"
let invalid = function Zenmux.Invalid_response _ -> true | _ -> false

(* A replacement curl checks the real HTTPS discovery and Chat executor,
   including both turns of a signed-reasoning tool continuation. *)
let fake_curl () =
  let config = ref [] in
  ...
  let state = Sys.getenv "PAVE_ZENMUX_FIXTURE_STATE" in
  ...
  let response = match step with
    | 0 ->
        has "url" Zenmux.models_url;
        has "request" "GET";
        has "max-filesize" (string_of_int Zenmux.max_response_bytes);
        assert (List.mem "Accept: application/json" (values "header"));
        {|{"object":"list","data":[{"id":"example/future-chat-model","object":"model","input_modalities":["text"],"capabilities":{"reasoning":true}}]}|}
    | 1 | 2 ->
        has "url" Zenmux.chat_url;
        has "request" "POST";
        ...
        if step = 1 then (
          ...
          {|{"id":"zenmux-1","object":"chat.completion","choices":[{"index":0,"finish_reason":"tool_calls","message":{"role":"assistant","content":null,"reasoning":"Use the calculator tool first.","reasoning_details":[{"type":"reasoning.text","text":"Use the calculator tool first.","signature":"signed-private-reasoning-proof","format":"anthropic-claude-v1","index":0,"provider_extension":{"preserve":true}}],"tool_calls":[{"id":"toolu_bdrk_01ZenMuxProof","type":"function","function":{"name":"multiply_seven","arguments":"{\"number\":6}"}}]}}]}|})
        else (
          (match field "messages" body with
           | `List [first; assistant; result] ->
               ...
               assert (field "reasoning" assistant = `String reasoning);
               assert (field "reasoning_details" assistant = details);
          ...
          {|{"id":"zenmux-2","object":"chat.completion","choices":[{"index":0,"finish_reason":"stop","message":{"role":"assistant","content":"Six times seven is 42."}}]}|})
...
let () =
  if Array.length Sys.argv >= 3 && Sys.argv.(1) = "--disable" && Sys.argv.(2) = "--config" then
    ...
    expect_error ((=) Zenmux.Invalid_credential)
      (Zenmux.discover ~http ~api_key:"bad\r\nAuthorization: Bearer stolen" ());
    assert (!calls = 1);
    List.iter (fun body -> expect_error invalid
      (Zenmux.discover ~http:(fun ~url:_ ~headers:_ -> Ok (200, body))
         ~api_key:key ())) [
    ...
    expect_error invalid (Zenmux.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (200,
        String.make (Zenmux.max_response_bytes + 1) 'x')) ~api_key:key ());
    expect_error invalid (Zenmux.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (200,
        Yojson.Basic.to_string (`Assoc ["object", `String "list";
          "data", `List (List.init (Zenmux.max_models + 1)
            (fun _ -> `Assoc ["id", `String model; "object", `String "model"]))])))
      ~api_key:key ());
    expect_error ((=) (Zenmux.Http_error 302)) (Zenmux.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (302, "redirect")) ~api_key:key ());
    assert (Zenmux.chat_headers ~endpoint:Zenmux.chat_url ~api_key:key =
      ["Authorization: Bearer " ^ key]);
    (match Zenmux.chat_headers ~endpoint:"https://evil.example/api/v1/chat/completions" ~api_key:key with
     | exception Invalid_argument _ -> ()
     | _ -> fail "credential escaped pinned host");
    (match Zenmux.chat_headers ~endpoint:Zenmux.chat_url ~api_key:"bad\nvalue" with
     | exception Invalid_argument _ -> ()
     | _ -> fail "header injection accepted");
    let directory = Filename.temp_file "pave-zenmux-" "" in
    ...
    Fun.protect ~finally:(fun () ->
      ...
      let call = match first.tool_calls with
        ...
        | _ -> fail "ZenMux tool call not decoded" in
    ...
    print_endline "ZenMux authenticated listing and signed tool continuation: ok")
