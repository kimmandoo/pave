module Stepfun = Pave.Stepfun_api
module Protocol = Pave.Protocol
let field = Protocol.member

let key = "international-stepfun-fixture-key"
let model = "example/available-chat-model"
let audio_model = "stepaudio-2.5-tts"
let call_id = "call_stepfun_47"
...
let fail reason = failwith ("StepFun fixture: " ^ reason)
...
let expect_error check = function
  ...
  | _ -> fail "unsafe StepFun listing accepted"
let invalid = function Stepfun.Invalid_response _ -> true | _ -> false

(* Fake curl inspects the actual production HTTP executor: one authenticated
   account listing, then two native Chat turns with a locally computed result. *)
let fake_curl () =
  let config = ref [] in
  ...
  let state = Sys.getenv "PAVE_STEPFUN_FIXTURE_STATE" in
  ...
  let response = match step with
    | 0 ->
        has "url" Stepfun.models_url;
        has "request" "GET";
        has "max-filesize" (string_of_int Stepfun.max_response_bytes);
        assert (List.mem "Accept: application/json" (values "header"));
        {|{"object":"list","data":[{"id":"example/available-chat-model","object":"model","created":1700000000,"owned_by":"stepai"},{"id":"stepaudio-2.5-tts","object":"model","created":1700000001,"owned_by":"stepai"}]}|}
    | 1 | 2 ->
        has "url" Stepfun.chat_url;
        has "request" "POST";
        ...
        (match body with
         | `Assoc fields ->
             assert (not (List.mem_assoc "reasoning_format" fields));
        ...
        if step = 1 then (
          ...
          {|{"object":"chat.completion","choices":[{"index":0,"finish_reason":"stop","message":{"role":"assistant","content":"","reasoning":"Calculating using the provided tool.","tool_calls":[{"id":"call_stepfun_47","type":"function","function":{"name":"multiply_seven","arguments":"{\"number\":6}"}}]}}]}|})
        else (
          (match field "messages" body with
           | `List [first; assistant; result] ->
               ...
               assert (field "reasoning" assistant =
                 `String "Calculating using the provided tool.");
...
let () =
  if Array.length Sys.argv >= 3 && Sys.argv.(1) = "--disable" && Sys.argv.(2) = "--config" then
    ...
    expect_error ((=) Stepfun.Invalid_credential)
      (Stepfun.discover ~http ~api_key:"bad\r\nAuthorization: Bearer stolen" ());
    assert (!calls = 1);
    List.iter (fun body -> expect_error invalid
      (Stepfun.discover ~http:(fun ~url:_ ~headers:_ -> Ok (200, body))
         ~api_key:key ())) [
      {|{"data":[{"id":"bad\nmodel","object":"model"}]}|};
      {|{"data":[{"id":null,"object":"model"}]}|};
      {|{"data":[{"id":"x","object":"not-model"}]}|};
      {|{"data":[{}]}|}; {|{"models":[]}|}; "not json"];
    expect_error invalid (Stepfun.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (200,
        String.make (Stepfun.max_response_bytes + 1) 'x')) ~api_key:key ());
    expect_error ((=) (Stepfun.Http_error 302)) (Stepfun.discover
      ~http:(fun ~url:_ ~headers:_ -> Ok (302, "redirect")) ~api_key:key ());
    assert (Stepfun.chat_headers ~endpoint:Stepfun.chat_url ~api_key:key =
      ["Authorization: Bearer " ^ key]);
    List.iter (fun endpoint ->
      match Stepfun.chat_headers ~endpoint ~api_key:key with
      | exception Invalid_argument _ -> ()
      | _ -> fail "credential escaped pinned international host") [
        "https://api.stepfun.com/v1/chat/completions";
        "https://evil.example/v1/chat/completions"];
    (match Stepfun.chat_headers ~endpoint:Stepfun.chat_url ~api_key:"bad\nvalue" with
     | exception Invalid_argument _ -> ()
     | _ -> fail "header injection accepted");
    let rejected_reply body =
      match Stepfun.parse_completion (Yojson.Basic.from_string body) with
      | exception Protocol.Invalid_response _ -> ()
      | _ -> fail "incomplete StepFun tool call accepted" in
    List.iter rejected_reply [
      {|{"choices":[{"finish_reason":"stop","message":{"content":"","tool_calls":[{"id":"call_1","type":"function","function":{"name":"calculate","arguments":"{"}}]}}]}|};
      {|{"choices":[{"finish_reason":"stop","message":{"content":"","tool_calls":[{"id":"call_1","type":"web_search","function":{"name":"calculate","arguments":"{}"}}]}}]}|};
      {|{"choices":[{"finish_reason":"stop","message":{"content":"","tool_calls":[{"id":"call_1","type":"function","function":{"name":"calculate","arguments":"[1]"}}]}}]}|};
      {|{"choices":[{"finish_reason":"stop","message":{"content":"","tool_calls":[{"id":"call_1","type":"function","function":{"arguments":"{}"}}]}}]}|}];
    let reasoning_reply = Stepfun.parse_completion
      (Yojson.Basic.from_string
        {|{"choices":[{"finish_reason":"stop","message":{"role":"assistant","content":"ok","reasoning_content":"thinking"}}]}|}) in
    assert (reasoning_reply.provider_state =
      Some (`Assoc ["reasoning_content", `String "thinking"]));
    let replay = Stepfun.request ~model [reasoning_reply] [] in
    (match field "messages" replay with
     | `List [message] ->
         assert (field "reasoning_content" message = `String "thinking")
     | _ -> fail "deepseek-style reasoning lost on replay");
    let directory = Filename.temp_file "pave-stepfun-" "" in
    ...
    Fun.protect ~finally:(fun () ->
      ...
      Unix.putenv "PAVE_STEPFUN_FIXTURE_STATE" "";
      List.iter (fun path -> if Sys.file_exists path then Sys.remove path) [binary; state];
      Unix.rmdir directory) (fun () ->
      Unix.putenv "PATH" (directory ^ ":" ^ old_path);
      Unix.putenv "PAVE_STEPFUN_FIXTURE_STATE" state;
      let discovered = match Pave.Model_discovery.discover
        ~provider:"stepfun" ~credential:(Pave.Model_discovery.Api_key key) () with
        | Ok ids when ids = [model; audio_model] -> model
        | _ -> fail "production model discovery lost unclassified IDs" in
      let config : Pave.Provider.config = {
        endpoint = Stepfun.chat_url; api_key = key; model = discovered;
        api = Pave.Provider.Stepfun_chat } in
      (match Pave.Provider.complete
        { config with endpoint = "https://api.stepfun.com/v1/chat/completions" }
        [Protocol.user "Do not leak credentials"] [] with
       | exception Pave.Provider.Provider_error _ -> ()
       | _ -> fail "credential sent to different StepFun platform");
      ...
      let call = match first.tool_calls with
        ...
        | _ -> fail "StepFun tool call not decoded for stop finish reason" in
      assert (first.provider_state = Some (`Assoc [
        "reasoning", `String "Calculating using the provided tool."]));
    ...
    print_endline "StepFun authenticated listing and native tool continuation: ok")
